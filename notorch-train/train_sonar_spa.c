/*
 * train_sonar_spa.c — Janus Sonar v2 + SPA contrastive (dual-loss), pure C.
 *
 * Architecture: same base as v2 (single 3.11M, DIM=160, L=6, H=5, HIDDEN=320).
 * Extension: Sentence Phonon Attention (SPA) as a contrastive auxiliary.
 *
 *   Per step:
 *     1. Two adjacent windows of the corpus: anchor + positive (offset by CTX).
 *     2. Base encoder runs over both → two final h[T, D] (after final RMSNorm).
 *     3. Sentence embedding sₐ, sₚ = h at last sentence-boundary token of each window
 *        (via nt_seq_row). If no boundary, fall back to last position.
 *     4. Negative sₙ = previous step's sₚ (memory bank, detached — copied by value).
 *     5. loss_spa = relu(margin + <sₐ, sₙ> - <sₐ, sₚ>)   — nt_triplet_loss
 *     6. loss_lm  = avg(CE(anchor), CE(positive))
 *     7. total    = loss_lm + 0.1 * loss_spa
 *
 * New notorch ops used: nt_seq_row, nt_triplet_loss. Both finite-diff verified.
 *
 *   make train_sonar_spa
 *   ./train_sonar_spa [steps] [lr]
 *   ./train_sonar_spa --resume [steps] [lr]
 */
#include "notorch.h"
#include <stdio.h>
#include <string.h>
...
#define EVAL_SEQS   16
#define CKPT_PREFIX "sonar_spa_v1_ckpt"

#define SPA_WEIGHT   0.1f   /* total = lm + SPA_WEIGHT * spa */
#define SPA_MARGIN   0.3f   /* triplet hinge margin */

typedef struct {
    nt_tensor *wte;
    struct {
        nt_tensor *rms1;
        nt_tensor *wq, *wk, *wv, *wvr, *wj, *wo;
        nt_tensor *wr;
        nt_tensor *rms2;
...
/* ── Forward: full base + final RMSNorm → hf; expose both hf and LM loss ── */

typedef struct { int loss_idx; int hf_idx; } FwdOut;

static FwdOut forward_lm_hf(Model* m, int* tokens, int* targets) {
    ...
    int lm_loss = nt_seq_cross_entropy(logits, tgt_i, CTX, VOCAB);

    FwdOut out = { lm_loss, hf };
    return out;
}

/* ── Sentence-boundary detection: last . ! ? or \n within [off, off+CTX) ── */
static int find_last_boundary(const nt_bpe* bpe, const int* encoded, int off) {
    int last = -1;
    for (int t = 0; t < CTX; t++) {
        int tok = encoded[off + t];
        if (tok < 0 || tok >= bpe->vocab_size) continue;
        int len = bpe->token_len[tok];
        for (int i = 0; i < len; i++) {
            unsigned char c = bpe->tokens[tok][i];
            if (c == '.' || c == '!' || c == '?' || c == '\n') { last = t; break; }
        }
    }
    return last;
...
/* ── Eval loss (LM only, like v2) ── */
static float eval_loss(Model* m, int* encoded, int n_tokens) {
    float total = 0; int count = 0;
    int stride = n_tokens / EVAL_SEQS; if (stride < 1) stride = 1;
    for (int s = 0; s < EVAL_SEQS; s++) {
        int off = s * stride;
        if (off + CTX + 1 > n_tokens) break;
        nt_tape_start();
        nt_train_mode(0);
        FwdOut fo = forward_lm_hf(m, encoded + off, encoded + off + 1);
        total += nt_tape_get()->entries[fo.loss_idx].output->data[0];
        count++;
        nt_tape_clear();
        nt_train_mode(1);
    }
...
int main(int argc, char** argv) {
    ...
    printf("════════════════════════════════════════════════════════\n");
    printf("  notorch — JANUS SONAR SPA v1 training (dual-loss)\n");
    ...
    printf("  SPA contrastive: triplet(sₐ, sₚ, sₙ_membank), margin=%.2f, weight=%.2f\n",
           SPA_MARGIN, SPA_WEIGHT);
    printf("  sₐ,sₚ = h[last_boundary] over adjacent CTX windows; sₙ = prev sₚ (detached)\n");
    ...
    printf("karpathy: %.1f epochs over %d steps (each step = 2 windows)\n",
           2.0f * (float)steps * CTX / n_tokens, steps);
    ...
    /* Memory bank for SPA negatives: last seen positive sent_emb */
    float neg_cache[DIM];
    ...
    double t0 = now_ms();
    float first_loss = 0;
    float ema_lm = 0, ema_spa = 0;  /* running averages for log */

    for (int step = 0; step < steps; step++) {
        float lr = nt_schedule_get_lr(&sched);

        /* Sample two adjacent windows, both fit */
        int max_start = n_tokens - 2 * CTX - 2;
        if (max_start < 1) { printf("corpus too small\n"); break; }
        int off_a = rand() % max_start;
        int off_p = off_a + CTX;

        /* Last-boundary position in each window — fallback to CTX-1 */
        int row_a = find_last_boundary(&bpe, encoded, off_a); if (row_a < 0) row_a = CTX - 1;
        int row_p = find_last_boundary(&bpe, encoded, off_p); if (row_p < 0) row_p = CTX - 1;

        nt_tape_start();

        FwdOut fa = forward_lm_hf(model, encoded + off_a, encoded + off_a + 1);
        FwdOut fp = forward_lm_hf(model, encoded + off_p, encoded + off_p + 1);

        int sent_a = nt_seq_row(fa.hf_idx, row_a, DIM);
        int sent_p = nt_seq_row(fp.hf_idx, row_p, DIM);

        int lm_sum    = nt_add(fa.loss_idx, fp.loss_idx);
        int lm_avg    = nt_scale(lm_sum, 0.5f);

        int total_idx;
        float spa_val = 0;
        if (neg_ready) {
            nt_tensor* neg_t = nt_tensor_new(DIM);
            memcpy(neg_t->data, neg_cache, DIM * sizeof(float));
            int sent_n   = nt_tape_record(neg_t, NT_OP_NONE, -1, -1, 0);
            nt_tensor_free(neg_t);

            int spa_idx  = nt_triplet_loss(sent_a, sent_p, sent_n, SPA_MARGIN);
            spa_val      = nt_tape_get()->entries[spa_idx].output->data[0];
            int spa_scl  = nt_scale(spa_idx, SPA_WEIGHT);
            total_idx    = nt_add(lm_avg, spa_scl);
        } else {
            total_idx = lm_avg;
        }

        float lm_val    = nt_tape_get()->entries[lm_avg].output->data[0];
        float total_val = nt_tape_get()->entries[total_idx].output->data[0];

        if (step == 0) { first_loss = lm_val; ema_lm = lm_val; ema_spa = spa_val; }
        else {
            ema_lm  = 0.98f * ema_lm  + 0.02f * lm_val;
            ema_spa = 0.98f * ema_spa + 0.02f * spa_val;
        }
        if (lm_val < best_loss) best_loss = lm_val;

        nt_tape_backward(total_idx);
        if (!nt_nan_guard_check(&guard)) { nt_tape_clear(); continue; }
        nt_tape_clip_grads(1.0f);
        nt_tape_chuck_step(lr, total_val);

        /* Update negative memory bank: current positive sent_emb (detached) */
        memcpy(neg_cache,
               nt_tape_get()->entries[sent_p].output->data,
               DIM * sizeof(float));
        neg_ready = 1;
        ...
        if ((step+1) % LOG_EVERY == 0 || step == 0) {
            printf("  step %5d/%d | train %.4f | lm %.4f | spa %.4f | ema_lm %.4f ema_spa %.4f | lr %.2e | %.1fs\n",
                   step+1, steps, total_val, lm_val, spa_val, ema_lm, ema_spa, lr, (now_ms()-t0)/1000.0);
            fflush(stdout);
        }
        if ((step+1) % CKPT_EVERY == 0) {
            float val = eval_loss(model, encoded, n_tokens);
            printf("  ──── ckpt %d | val %.4f | best %.4f\n", step+1, val, best_loss);
